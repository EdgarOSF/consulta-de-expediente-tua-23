...
def generate_qr(num_juicioa, num_juiciob):
    qrcode = segno.make(f'http://192.168.0.99:8080/?num_juicio1={num_juicioa}&num_juicio2={num_juiciob}')
    qrcode.save(f'expediente_{num_juicioa}-{num_juiciob}_{uuid.uuid1()}.png')


def get_expediente_id(request, num_juicioa, num_juiciob):

    numero_juicio_a = num_juicioa
    numero_juicio_b = num_juiciob
...
def get_expediente_detail(request, num_juicioa, num_juiciob):

    expediente_id = get_expediente_id(request, num_juicioa, num_juiciob)
...
def expediente_detail(request):
    ...
    #     return render(request, 'webpage/expediente_detail.html', context)
    if request.method == 'GET':
        num_juicio1 = request.GET.get('num_juicio1')
        num_juicio2 = request.GET.get('num_juicio2')
        context = get_expediente_detail(request, num_juicio1, num_juicio2)
        if context:
            generate_qr(num_juicio1, num_juicio2)
        return render(request, 'webpage/expediente_detail.html', context)
